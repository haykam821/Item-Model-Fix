...
public class ClothConfigFactory implements ConfigScreenFactory<Screen> {
    ...
    public Screen create(Screen parent) {
        ...
        general.addEntry(entryBuilder.startEnumSelector(Text.translatable("option.itemmodelfix.generation_type"), ItemModelGenerationType.class, config.getOptions().generationType)
                ...
                .setEnumNameProvider((value) -> Text.translatable("option.itemmodelfix.generation_type." + value.name().toLowerCase(Locale.ROOT)))
                .setTooltipSupplier((value) -> Optional.ofNullable(ParsingUtil.parseNewlines("option.itemmodelfix.generation_type." + value.name().toLowerCase(Locale.ROOT) + ".tooltip")))
